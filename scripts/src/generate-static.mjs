...
const categories = [
  { id: 1, name: "Injectables", slug: "injectables", description: "Various injectable pharmaceutical formulations." },
  { id: 2, name: "Tablet/capsules", slug: "tablet-capsules", description: "Solid oral dosage forms including tablets and capsules." },
  { id: 3, name: "APIs(Active Pharmaceutical Ingredients)", slug: "apis", description: "Active Pharmaceutical Ingredients for pharmaceutical manufacturing." },
];
...
function generateSlug(text) {
  return text.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') + '-' + (++pIdx);
}

// Parse Injectables
let injectablesCount = 0;
injectablesRaw.trim().split('\n').forEach(line => {
  ...
  if (parts.length >= 4) {
    products.push({
      ...
      name: parts[2].trim(),
      slug: generateSlug(parts[2].trim()),
...
tabletsRaw.trim().split('\n').forEach(line => {
  ...
  if (parts.length >= 4) {
    products.push({
      ...
      name: parts[3].trim(),
      slug: generateSlug(parts[3].trim()),
...
for (let i = 0; i < apisLines.length; i += 3) {
  if (i + 2 < apisLines.length) {
    products.push({
      ...
      slug: generateSlug(apisLines[i + 1].trim()),
...
categories[0].productCount = injectablesCount;
categories[1].productCount = tabletsCount;
categories[2].productCount = apiCount;
...
console.log("Statically generated into", outputPath);
